...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_STARCOIN_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_starcoin_bridge_message");
...
) -> StarcoinResult<Option<SequenceNumber>> {
    Ok(object_store
...
/// Rust version of the Move starcoin::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> StarcoinResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, StarcoinError> {
...
pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, StarcoinError> {
    let wrapper = get_bridge_wrapper(object_store)?;
    let id = wrapper.version.id.id;
    let version = wrapper.version.version;
    match version {
        1 => {
            let field = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    StarcoinError::StarcoinBridgeReadError(format!(
                ...
                .ok_or_else(|| {
                    StarcoinError::StarcoinBridgeReadError(format!(
                        "Bridge inner object not found for version {}",
                        version
                    ))
                })?;
            Ok(Bridge::V1(field.value))
        }
        _ => Err(StarcoinError::StarcoinBridgeReadError(format!(
            "Unsupported StarcoinBridge version: {}",
            version
        ))),
    }
}
...
impl BridgeTrait for BridgeInnerV1 {
    ...
    fn try_into_bridge_summary(self) -> StarcoinResult<BridgeSummary> {
        ...
        Ok(BridgeSummary {
            ...
            bridge_records_id: self.bridge_records.id.id,
            limiter,
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> StarcoinResult<bool> {
    match get_bridge(object_store) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(StarcoinError::StarcoinBridgeReadError(..)) => Ok(false),
        Err(other) => Err(other),
    }
}
