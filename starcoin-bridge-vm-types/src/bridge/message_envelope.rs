...
use super::messages_checkpoint::CheckpointSequenceNumber;
use fastcrypto::traits::KeyPair;

// Placeholder type - only needed for specific impl block
...
impl Message for SenderSignedData {
    type DigestType = super::base_types::TransactionDigest;
    const SCOPE: IntentScope = IntentScope::SenderSignedTransaction;

    fn digest(&self) -> Self::DigestType {
        // Placeholder implementation
        super::base_types::TransactionDigest::default()
    }
}
...
impl Envelope<SenderSignedData, AuthoritySignInfo> {
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> StarcoinResult<()> {
        self.auth_signature.verify_secure(
            self.data(),
            Intent::starcoin_bridge_app(IntentScope::SenderSignedTransaction),
            committee,
        )
    }
}

impl<T> Envelope<T, AuthorityQuorumSignInfo>
where
    T: Message + Serialize,
{
    pub fn new(
        ...
        let cert = Self {
            digest: OnceCell::new(),
            data,
            auth_signature: AuthorityQuorumSignInfo::new_from_auth_sign_infos(
        ...
        let signatures = keypairs
            ...
            .map(|keypair| {
                AuthoritySignInfo::new(
                    ...
                    &data,
                    &Intent::starcoin_bridge_app(T::SCOPE),
                    authority_public_key_bytes(keypair.public()),
                    keypair,
                )
            })
            .collect();
...
impl<T: Message> VerifiedEnvelope<T, CertificateProof> {
    pub fn new_from_certificate(
        ...
        let sig_bytes = bcs::to_bytes(&auth_signature).unwrap_or_default();
        VerifiedEnvelope::new_unchecked(Envelope {
            digest,
            data,
            auth_signature: CertificateProof::new_from_cert_sig(sig_bytes),
        })
    }
    ...
    pub fn new_system(transaction: VerifiedEnvelope<T, EmptySignInfo>, _epoch: EpochId) -> Self {
        let inner = transaction.into_inner();
        let Envelope {
            digest,
            data,
        ...
        VerifiedEnvelope::new_unchecked(Envelope {
            digest,
            data,
            auth_signature: CertificateProof::new_system(),
        })
    }
    ...
    pub fn new_from_quorum_execution(
        ...
        let epoch_bytes = bcs::to_bytes(&epoch).unwrap_or_default();
        VerifiedEnvelope::new_unchecked(Envelope {
            digest,
            data,
            auth_signature: CertificateProof::QuorumExecuted(epoch_bytes),
        })
    }
    ...
    pub fn new_from_consensus(
        ...
        VerifiedEnvelope::new_unchecked(Envelope {
            digest,
            data,
            auth_signature: CertificateProof::new_from_consensus(epoch, 0),
        })
    }
