...
// Simplified version for rustls 0.21 compatibility
// Only core certificate generation functionality is retained
mod certgen;
...
pub const STARCOIN_VALIDATOR_SERVER_NAME: &str = "starcoin";

// Simplified certificate creation - only what's actually used in the codebase
...
mod tests {
    ...
    fn verify_allowall() {
        ...
        let random_cert_bob =
            SelfSignedCertificate::new(allowed.private(), STARCOIN_VALIDATOR_SERVER_NAME);
        let random_cert_alice =
            SelfSignedCertificate::new(disallowed.private(), STARCOIN_VALIDATOR_SERVER_NAME);

        let verifier = ClientCertVerifier::new(AllowAll, STARCOIN_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_server_cert() {
        ...
        let verifier =
            ServerCertVerifier::new(allowed_public_key, STARCOIN_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_hashset() {
        ...
        let allowed_cert = SelfSignedCertificate::new(allowed.private(), STARCOIN_VALIDATOR_SERVER_NAME);
        ...
        let verifier =
            ClientCertVerifier::new(allowlist.clone(), STARCOIN_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn invalid_server_name() {
        ...
        let cert = SelfSignedCertificate::new(keypair.private(), "not-starcoin");

        let allowlist = AllowPublicKeys::new(BTreeSet::from([public_key.clone()]));
        let client_verifier =
            ClientCertVerifier::new(allowlist.clone(), STARCOIN_VALIDATOR_SERVER_NAME.to_string());

        // Allowed public key but the server-name in the cert is not the required "starcoin"
        ...
        let server_verifier =
            ServerCertVerifier::new(public_key, STARCOIN_VALIDATOR_SERVER_NAME.to_string());
    ...
    async fn axum_acceptor() {
        ...
        let client_certificate =
            SelfSignedCertificate::new(client_keypair.private(), STARCOIN_VALIDATOR_SERVER_NAME);
