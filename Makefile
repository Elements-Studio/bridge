...
deploy-starcoin-contracts: build-starcoin-contracts ## Deploy Move contracts + initialize committee (full automation)
	...
	if command -v jq >/dev/null 2>&1; then \
		echo "$(YELLOW)Exporting default account private key...$(NC)" && \
		ACCOUNT_EXPORT_OUTPUT=$$($(STARCOIN_PATH) -c $(STARCOIN_RPC) account export $$DEFAULT_ACCOUNT 2>&1); \
		EXPORT_EXIT_CODE=$$?; \
		if [ $$EXPORT_EXIT_CODE -eq 0 ]; then \
			JSON_START_LINE=$$(echo "$$ACCOUNT_EXPORT_OUTPUT" | grep -n "^{" | tail -1 | cut -d: -f1); \
			if [ -n "$$JSON_START_LINE" ]; then \
				JSON_OUTPUT=$$(echo "$$ACCOUNT_EXPORT_OUTPUT" | sed -n "$${JSON_START_LINE},\$$p"); \
			else \
				JSON_OUTPUT=$$(echo "$$ACCOUNT_EXPORT_OUTPUT" | tail -10); \
			fi; \
			PRIVATE_KEY=$$(echo "$$JSON_OUTPUT" | jq -r '.ok.private_key // empty' 2>/dev/null); \
			if [ -n "$$PRIVATE_KEY" ] && [ "$$PRIVATE_KEY" != "null" ] && [ "$$PRIVATE_KEY" != "empty" ]; then \
				echo "$(GREEN)✓ Private key extracted$(NC)"; \
			else \
				echo "$(YELLOW)⚠ Failed to extract private_key from account export output$(NC)"; \
				echo "$(YELLOW)   Continuing without private key in config...$(NC)"; \
				PRIVATE_KEY=""; \
			fi; \
		else \
			echo "$(YELLOW)⚠ Failed to export account: $$DEFAULT_ACCOUNT (exit code: $$EXPORT_EXIT_CODE)$(NC)"; \
			echo "$(YELLOW)   Continuing without private key in config...$(NC)"; \
		fi; \
	else \
		echo "$(YELLOW)⚠ jq is not installed, skipping private key extraction$(NC)"; \
		echo "$(YELLOW)   Continuing without private key in config...$(NC)"; \
	fi && \
	...
	if [ -n "$$PRIVATE_KEY" ]; then \
		echo "" >> bridge-config/cli-config.yaml && \
		echo "# Bridge proxy private key (Ed25519) for Starcoin" >> bridge-config/cli-config.yaml && \
		echo "starcoin-bridge-proxy-private-key: \"$$PRIVATE_KEY\"" >> bridge-config/cli-config.yaml; \
	fi && \
	echo "" >> bridge-config/cli-config.yaml && \
