...
pub const ADD_TOKENS_ON_STARCOIN_PATH: &str =
...
async fn handle_add_tokens_on_starcoin(
    ...
    let future = async {
        ...
        if !chain_id.is_starcoin_bridge_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_starcoin only expects Starcoin chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_starcoin", future).await
}
...
async fn handle_add_tokens_on_evm(
    ...
    let future = async {
        ...
        if chain_id.is_starcoin_bridge_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Starcoin chain id".to_string(),
            ));
        }
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_starcoin_bridge_decimals = token_starcoin_bridge_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token starcoin decimals: {:?}",
                        err
                    ))
                })
            })
