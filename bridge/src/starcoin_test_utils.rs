...
use starcoin_vm_types::transaction::{Package, ScriptFunction, SignedUserTransaction, TransactionPayload};
...
pub fn load_package_from(path: &str) -> Result<Package> {
    let blob_content = fs::read(path)
        .with_context(|| format!("Failed to read blob from {}", path))?;
    bcs_ext::from_bytes(&blob_content)
...
impl BridgeDeploymentBuilder {
    ...
    pub fn build_register_committee_transaction(&mut self, url: &str) -> Result<SignedUserTransaction> {
        let bridge_address = self.bridge_address()?;
    ...
    pub fn build_setup_token_transaction(&mut self, token_name: &str) -> Result<SignedUserTransaction> {
        let bridge_address = self.bridge_address()?;
