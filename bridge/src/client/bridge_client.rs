...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::StarcoinToEthBridgeAction(e) => format!(
                "sign/bridge_tx/starcoin/eth/{}/{}",
                Hex::encode(e.starcoin_bridge_tx_digest),
                e.starcoin_bridge_tx_event_index
            ),
            BridgeAction::EthToStarcoinBridgeAction(e) => format!(
                "sign/bridge_tx/eth/starcoin/{}/{}",
            ...
            BridgeAction::AddTokensOnStarcoinAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_starcoin/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_addresses = a
                    ...
                    .collect::<Vec<_>>()
                    .join(",");
                let token_starcoin_bridge_decimals = a
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_starcoin_bridge_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    use crate::{
        ...
        test_utils::{get_test_authority_and_key, get_test_starcoin_bridge_to_eth_bridge_action},
    ...
    async fn test_bridge_client() {
        ...
        // Ok
        authority.base_url = "https://foo.starcoinbridge.io".to_string();
        let committee = Arc::new(BridgeCommittee::new(vec![authority.clone()]).unwrap());
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        let action2 = get_test_starcoin_bridge_to_eth_bridge_action(
            Some(tx_digest),
            Some(event_idx),
            Some(2),
            Some(200),
