...
use crate::abi::{eth_starcoin_bridge, EthStarcoinBridge};
use crate::client::bridge_authority_aggregator::BridgeAuthorityAggregator;
use crate::crypto::BridgeAuthorityKeyPair;
use crate::e2e_tests::test_utils::TestClusterWrapperBuilder;
use crate::e2e_tests::test_utils::{
    get_signatures, initiate_bridge_erc20_to_starcoin, initiate_bridge_eth_to_starcoin,
    initiate_bridge_starcoin_bridge_to_eth, send_eth_tx_and_get_tx_receipt, BridgeTestClusterBuilder,
};
use crate::eth_transaction_builder::build_eth_transaction;
use crate::events::{
    StarcoinBridgeEvent, StarcoinToEthTokenBridgeV1, TokenTransferApproved, TokenTransferClaimed,
};
...
use std::sync::Arc;
use starcoin_bridge_json_rpc_types::{StarcoinExecutionStatus, StarcoinTransactionBlockEffectsAPI};
...
async fn test_bridge_from_eth_to_starcoin_bridge_to_eth() {
    telemetry_subscribers::init_for_testing();

    let eth_chain_id = BridgeChainId::EthCustom as u8;
    let starcoin_bridge_chain_id = BridgeChainId::StarcoinCustom as u8;
    ...
    let amount = 42;
    let starcoin_bridge_amount = amount * 100_000_000;

    initiate_bridge_eth_to_starcoin(&bridge_test_cluster, amount, 0)
        .await
        .unwrap();
    ...
    let eth_coin = bridge_test_cluster
        ...
        .coin_read_api()
        .get_all_coins(starcoin_bridge_address, None, None)
        .await
        .unwrap()
        .data
        .iter()
        ...
        .clone();
    assert_eq!(eth_coin.balance, starcoin_bridge_amount);
    info!(
        "[Timer] Eth to Starcoin bridge transfer finished in {:?}",
        timer.elapsed()
    );
    let timer = std::time::Instant::now();
    ...
    let starcoin_bridge_to_eth_bridge_action = initiate_bridge_starcoin_bridge_to_eth(
        &bridge_test_cluster,
        eth_address_1,
        eth_coin.object_ref(),
        nonce,
    ...
    let events = bridge_test_cluster
        .new_bridge_events(
            HashSet::from_iter([
                StarcoinToEthTokenBridgeV1.get().unwrap().clone(),
                TokenTransferApproved.get().unwrap().clone(),
                TokenTransferClaimed.get().unwrap().clone(),
            ]),
            true,
    ...
    assert_eq!(events.len(), 2);
    info!(
        "[Timer] Starcoin to Eth bridge transfer approved in {:?}",
        timer.elapsed()
    );
    let timer = std::time::Instant::now();
    ...
    let parsed_msg = bridge_test_cluster
        .bridge_client()
        .get_parsed_token_transfer_message(starcoin_bridge_chain_id, nonce)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(parsed_msg.source_chain as u8, starcoin_bridge_chain_id);
    assert_eq!(parsed_msg.seq_num, nonce);
    assert_eq!(
        parsed_msg.parsed_payload.sender_address,
        starcoin_bridge_address.to_vec()
    );
    assert_eq!(
    ...
    assert_eq!(parsed_msg.parsed_payload.amount, starcoin_bridge_amount);

    let message: eth_starcoin_bridge::Message = starcoin_bridge_to_eth_bridge_action.try_into().unwrap();
    let signatures = get_signatures(bridge_test_cluster.bridge_client(), nonce, starcoin_bridge_chain_id).await;

    let eth_starcoin_bridge = EthStarcoinBridge::new(
        bridge_test_cluster.contracts().starcoin_bridge,
        eth_signer.clone().into(),
    );
    let call = eth_starcoin_bridge.transfer_bridged_tokens_with_signatures(signatures, message);
    let eth_claim_tx_receipt = send_eth_tx_and_get_tx_receipt(call).await;
    assert_eq!(eth_claim_tx_receipt.status.unwrap().as_u64(), 1);
    info!(
        "[Timer] Starcoin to Eth bridge transfer claimed in {:?}",
        timer.elapsed()
    );
...
// Test add new coins on both Starcoin and Eth
// Also test bridge ndoe handling `NewTokenEvent`
#[tokio::test(flavor = "multi_thread", worker_threads = 8)]
async fn test_add_new_coins_on_starcoin_bridge_and_eth() {
    telemetry_subscribers::init_for_testing();
    let mut bridge_test_cluster = BridgeTestClusterBuilder::new()
        .with_eth_env(true)
    ...
    // Register tokens on Starcoin
    let token_id = 5;
    let token_starcoin_bridge_decimal = 9; // this needs to match ka.move
    let token_price = 10000;
    let sender = bridge_test_cluster.starcoin_bridge_user_address();
    info!("Published new token");
    let starcoin_bridge_action = publish_and_register_coins_return_add_coins_on_starcoin_bridge_action(
    ...
    let eth_action = BridgeAction::AddTokensOnEvmAction(AddTokensOnEvmAction {
        ...
        token_starcoin_bridge_decimals: vec![token_starcoin_bridge_decimal],
    ...
    bridge_test_cluster.set_approved_governance_actions_for_next_start(vec![
        vec![starcoin_bridge_action.clone(), eth_action.clone()],
        vec![starcoin_bridge_action.clone()],
    ...
    let certified_starcoin_bridge_action = agg
        .request_committee_signatures(starcoin_bridge_action)
        .await
        .expect("Failed to request committee signatures for AddTokensOnStarcoinAction");
    ...
    let tx = build_add_tokens_on_starcoin_bridge_transaction(
        ...
        &bridge_test_cluster
            ...
            .unwrap()
            .unwrap(),
        certified_starcoin_bridge_action,
        bridge_arg,
        1000,
    )
    .unwrap();
    ...
    assert_eq!(effects.status(), &StarcoinExecutionStatus::Success);
    assert!(response.events.unwrap().data.iter().any(|e| {
        let starcoin_bridge_event = StarcoinBridgeEvent::try_from_starcoin_bridge_event(e).unwrap().unwrap();
        match starcoin_bridge_bridge_event {
            StarcoinBridgeEvent::NewTokenEvent(e) => {
    ...
    info!("Approved new token on Starcoin");
    ...
    initiate_bridge_erc20_to_starcoin(
        &bridge_test_cluster,
        100,
