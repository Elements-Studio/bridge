...
use crate::abi::{EthBridgeCommittee, EthBridgeEvent, EthERC20, EthStarcoinBridge, EthStarcoinBridgeEvents};
...
use crate::starcoin_bridge_transaction_builder::build_add_tokens_on_starcoin_bridge_transaction;
use crate::starcoin_bridge_transaction_builder::build_committee_register_transaction;
...
use crate::types::{BridgeAction, BridgeActionStatus, StarcoinToEthBridgeAction};
use crate::utils::get_eth_signer_client;
use crate::utils::publish_and_register_coins_return_add_coins_on_starcoin_bridge_action;
...
use starcoin_bridge_json_rpc_types::StarcoinTransactionBlockResponseQuery;
use starcoin_bridge_json_rpc_types::TransactionFilter;
...
use starcoin_bridge_types::bridge::{
    get_bridge, get_bridge_obj_initial_shared_version, BridgeChainId, BridgeSummary, BridgeTrait,
    BRIDGE_MODULE_NAME, TOKEN_ID_BTC, TOKEN_ID_ETH, TOKEN_ID_USDC, TOKEN_ID_USDT,
};
use starcoin_bridge_types::committee::{ProtocolVersion, TOTAL_VOTING_POWER};
use starcoin_bridge_types::crypto::get_key_pair;
use starcoin_bridge_types::object::Object;
...
use crate::config::{BridgeNodeConfig, EthConfig, StarcoinConfig};
...
use starcoin_bridge_config::local_ip_utils::get_available_port;
...
const STARCOIN_BRIDGE_NAME: &str = "StarcoinBridge";
...
pub struct BridgeTestCluster {
    ...
    bridge_client: StarcoinBridgeClient,
    ...
    starcoin_bridge_chain_id: BridgeChainId,
}
...
impl BridgeTestClusterBuilder {
    ...
    pub fn with_starcoin_bridge_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.starcoin_bridge_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client =
            StarcoinBridgeClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
                .await
                .unwrap();
        info!(
            "Bridge committee: {:?}",
        ...
        BridgeTestCluster {
            ...
            starcoin_bridge_chain_id: self.starcoin_bridge_chain_id,
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &StarcoinBridgeClient {
        &self.bridge_client
    }

    pub fn starcoin_bridge_client(&self) -> &StarcoinClient {
        &self.test_cluster.inner.fullnode_handle.starcoin_bridge_client
    }

    pub fn starcoin_bridge_user_address(&self) -> StarcoinAddress {
        self.test_cluster.inner.get_address_0()
    }

    pub fn starcoin_bridge_chain_id(&self) -> BridgeChainId {
        self.starcoin_bridge_chain_id
    }
    ...
    pub fn starcoin_bridge_address(&self) -> String {
        self.eth_environment.contracts().starcoin_bridge_addrress_hex()
    }
    ...
    pub fn starcoin_bridge_rpc_url(&self) -> String {
    ...
    ) -> StarcoinTransactionBlockResponse {
        self.test_cluster
            .inner
    ...
    pub async fn new_bridge_transactions(
        &mut self,
        assert_success: bool,
    ) -> Vec<StarcoinTransactionBlockResponse> {
        let resps = self
            .starcoin_bridge_client()
            .read_api()
            .query_transaction_blocks(
                StarcoinTransactionBlockResponseQuery {
                    filter: Some(TransactionFilter::InputObject(STARCOIN_BRIDGE_OBJECT_ID)),
                    options: Some(StarcoinTransactionBlockResponseOptions::full_content()),
                },
...
impl DeployedSolContracts {
    ...
    pub fn starcoin_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.starcoin_bridge)
    }
}
...
struct SolDeployConfig {
    ...
    token_ids: Vec<u64>,
    starcoin_bridge_decimals: Vec<u64>,
...
pub(crate) async fn deploy_sol_contract(
    ...
    let deploy_config = SolDeployConfig {
        ...
        starcoin_bridge_decimals: vec![],     // this is set up in the deploy script
    ...
    let contracts = DeployedSolContracts {
        starcoin_bridge: deployed_contracts.remove(STARCOIN_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_starcoin_bridge_decimal = config.token_starcoin_bridge_decimal_of(token_id).call().await.unwrap();
        let token_price = config.token_price_of(token_id).call().await.unwrap();
        (token_address, token_starcoin_bridge_decimal, token_price)
    }
}
...
pub async fn get_signatures(
    ...
    nonce: u64,
    starcoin_bridge_chain_id: u8,
) -> Vec<Bytes> {
    let sigs = starcoin_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(starcoin_bridge_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = StarcoinAddress::from(k.public());
        ...
        for (node, kp) in reordered_nodes.zip(self.bridge_authority_keys.iter()) {
            let validator_address = node.config().starcoin_bridge_address();
            ...
            tasks.push(async move {
                api_clone
                    .execute_transaction_block(
                        tx,
                        StarcoinTransactionBlockResponseOptions::new().with_effects(),
                        None,
                    )
                    .await
            });
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_starcoin_bridge_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
            ...
            let tx = build_add_tokens_on_starcoin_bridge_transaction(
                sender_address,
                &test_cluster
                    .wallet
        ...
        async fn await_committee_register_tasks(
            ...
            tasks: Vec<
                impl Future<Output = Result<StarcoinTransactionBlockResponse, starcoin_bridge_sdk::error::Error>>,
            >,
        ) {
            ...
            for response in responses {
                if response.unwrap().effects.unwrap().status() != &StarcoinExecutionStatus::Success {
                    has_failure = true;
                }
            }
            if has_failure {
...
pub async fn initiate_bridge_eth_to_starcoin(
    ...
    let starcoin_bridge_address = bridge_test_cluster.starcoin_bridge_user_address();
    let starcoin_bridge_chain_id = bridge_test_cluster.starcoin_bridge_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    let token_id = TOKEN_ID_ETH;

    let starcoin_bridge_amount = (U256::from(amount) * U256::exp10(8)).as_u64(); // DP for Ether on Starcoin

    let eth_tx = deposit_native_eth_to_sol_contract(
        &eth_signer,
        bridge_test_cluster.contracts().starcoin_bridge,
    ...
    let eth_bridge_event = tx_receipt
        ...
        .unwrap();
    let EthBridgeEvent::EthStarcoinBridgeEvents(EthStarcoinBridgeEvents::TokensDepositedFilter(
        eth_bridge_event,
    )) = eth_bridge_event
    else {
        unreachable!();
    ...
    assert_eq!(eth_bridge_event.destination_chain_id, starcoin_bridge_chain_id as u8);
    assert_eq!(eth_bridge_event.token_id, token_id);
    assert_eq!(eth_bridge_event.starcoin_bridge_adjusted_amount, starcoin_bridge_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(eth_bridge_event.recipient_address, starcoin_bridge_address.to_vec());
    info!(
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Starcoin bridge transfer claimed");
    })
}

pub async fn initiate_bridge_starcoin_bridge_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    starcoin_bridge_amount: u64,
) -> Result<StarcoinToEthBridgeAction, anyhow::Error> {
    let bridge_object_arg = bridge_test_cluster
        .bridge_client()
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let starcoin_bridge_client = bridge_test_cluster.starcoin_bridge_client();
    ...
    let resp = match deposit_eth_to_starcoin_bridge_package(
        starcoin_bridge_client,
        starcoin_bridge_address,
    ...
    .await
    {
        Ok(resp) => {
            if !resp.status_ok().unwrap() {
                return Err(anyhow!("Starcoin TX error"));
            } else {
                resp
            }
        }
    ...
    let starcoin_bridge_events = resp.events.unwrap().data;
    let bridge_event = starcoin_bridge_events
        .iter()
        .filter_map(|e| {
            let starcoin_bridge_event = StarcoinBridgeEvent::try_from_starcoin_bridge_event(e).unwrap()?;
            starcoin_bridge_bridge_event.try_into_bridge_action(e.id.tx_digest, e.id.event_seq as u16)
        })
        .find_map(|e| {
            if let BridgeAction::StarcoinToEthBridgeAction(a) = e {
                Some(a)
            } else {
                None
            }
    ...
    assert_eq!(bridge_event.starcoin_bridge_event.nonce, nonce);
    assert_eq!(
        bridge_event.starcoin_bridge_event.starcoin_bridge_chain_id,
        bridge_test_cluster.starcoin_bridge_chain_id()
    );
    assert_eq!(
        bridge_event.starcoin_bridge_event.eth_chain_id,
        bridge_test_cluster.eth_chain_id()
    );
    assert_eq!(bridge_event.starcoin_bridge_event.starcoin_bridge_address, starcoin_bridge_address);
    assert_eq!(bridge_event.starcoin_bridge_event.eth_address, eth_address);
    assert_eq!(bridge_event.starcoin_bridge_event.token_id, TOKEN_ID_ETH);
    assert_eq!(
        bridge_event.starcoin_bridge_event.amount_starcoin_bridge_adjusted,
        starcoin_bridge_amount
    );
    ...
    .await
    .unwrap();
    info!("Starcoin to Eth bridge transfer approved.");
...
async fn wait_for_transfer_action_status(
    ...
    loop {
        ...
        let res = starcoin_bridge_client
...
async fn deposit_eth_to_starcoin_bridge_package(
    starcoin_bridge_client: &StarcoinClient,
...
) -> Result<StarcoinTransactionBlockResponse, anyhow::Error> {
    ...
    builder.programmable_move_call(
        ...
        vec![starcoin_bridge_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
        vec![arg_bridge, arg_target_chain, arg_target_address, arg_token],
    );

    let pt = builder.finish();
    let gas_object_ref = wallet_context
        .get_one_gas_object_owned_by_address(starcoin_bridge_address)
        .await
        .unwrap()
        .unwrap();
...
pub async fn initiate_bridge_erc20_to_starcoin(
    bridge_test_cluster: &BridgeTestCluster,
    amount_u64: u64,
    ...
    let starcoin_bridge_amount = amount.as_u64();
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().starcoin_bridge, amount);
    ...
    let starcoin_bridge_recipient_address = bridge_test_cluster.starcoin_bridge_user_address();
    let starcoin_bridge_chain_id = bridge_test_cluster.starcoin_bridge_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    ...
    let contract = EthStarcoinBridge::new(
        bridge_test_cluster.contracts().starcoin_bridge,
    ...
    let deposit_call = contract.bridge_erc20(
        ...
        starcoin_bridge_chain_id as u8,
    );
    ...
    assert_eq!(
        ...
        starcoin_bridge_recipient_address.to_vec()
    );
    info!(
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to Starcoin bridge transfer claimed"
        );
    })
}
...
pub(crate) async fn deposit_native_eth_to_sol_contract(
    ...
    let contract = EthStarcoinBridge::new(contract_address, signer.clone().into());
    let starcoin_bridge_recipient_address = starcoin_bridge_recipient_address.to_vec().into();
    let amount = U256::from(amount) * U256::exp10(18); // 1 ETH
    contract
        .bridge_eth(starcoin_bridge_recipient_address, starcoin_bridge_chain_id as u8)
        .value(amount)
}
