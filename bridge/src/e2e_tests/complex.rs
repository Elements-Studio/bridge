...
use crate::e2e_tests::test_utils::{
    initiate_bridge_eth_to_starcoin, initiate_bridge_starcoin_bridge_to_eth, BridgeTestClusterBuilder,
};
...
async fn test_starcoin_bridge_paused() {
    ...
    let starcoin_bridge_token_type_tags = bridge_client.get_token_id_map().await.unwrap();

    // verify bridge are not paused
    assert!(!bridge_client.get_bridge_summary().await.unwrap().is_frozen);

    // try bridge from eth and verify it works on starcoin
    initiate_bridge_eth_to_starcoin(&bridge_test_cluster, 10, 0)
        .await
        .unwrap();
    // verify Eth was transferred to Starcoin address
    let eth_coin_type = starcoin_bridge_token_type_tags.get(&TOKEN_ID_ETH).unwrap();
    let eth_coin = bridge_client
        .starcoin_bridge_client()
        .coin_read_api()
        .get_coins(starcoin_bridge_address, Some(eth_coin_type.to_string()), None, None)
        .await
        .unwrap()
        .data;
    ...
    // execute pause bridge on starcoin
    ...
    // Transfer from eth to starcoin should fail on Starcoin
    let eth_to_starcoin_bridge_action = initiate_bridge_eth_to_starcoin(&bridge_test_cluster, 10, 1).await;
    assert!(eth_to_starcoin_bridge_action.is_err());
    // message should not be recorded on Starcoin when the bridge is paused
    ...
    // Transfer from Starcoin to eth should fail
