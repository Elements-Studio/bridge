    ...
    C: StarcoinClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let Self {
            starcoin_bridge_client,
            mut starcoin_bridge_monitor_rx,
            mut eth_monitor_rx,
            bridge_auth_agg,
            bridge_paused_watch_tx,
            starcoin_bridge_token_type_tags,
        ...
        let mut latest_token_config = (*starcoin_bridge_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                starcoin_bridge_event = starcoin_bridge_monitor_rx.recv() => {
                    if let Some(starcoin_bridge_event) = starcoin_bridge_event {
                        Self::handle_starcoin_bridge_events(
                        ...
                        panic!("BridgeMonitor starcoin events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_starcoin_bridge_events(
        event: StarcoinBridgeEvent,
        starcoin_bridge_client: &Arc<StarcoinClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        starcoin_bridge_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received StarcoinBridgeEvent: {:?}", event);
        macro_rules! bump_starcoin_bridge_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "starcoin"])
                    .inc();
            };
        }

        match event {
            StarcoinBridgeEvent::StarcoinToEthTokenBridgeV1(_) => (),
            StarcoinBridgeEvent::TokenTransferApproved(_) => (),
            StarcoinBridgeEvent::TokenTransferClaimed(_) => (),
            StarcoinBridgeEvent::TokenTransferAlreadyApproved(_) => (),
            StarcoinBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            StarcoinBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            StarcoinBridgeEvent::EmergencyOpEvent(event) => {
                bump_starcoin_bridge_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            StarcoinBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_starcoin_bridge_counter!("committee_member_registered");
            }
            StarcoinBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_starcoin_bridge_counter!("committee_updated");
            }

            StarcoinBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_starcoin_bridge_counter!("committee_member_url_updated");
            ...
            StarcoinBridgeEvent::TokenRegistrationEvent(_) => {
                bump_starcoin_bridge_counter!("new_token_registered");
            }

            StarcoinBridgeEvent::NewTokenEvent(event) => {
                bump_starcoin_bridge_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    starcoin_bridge_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            StarcoinBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_starcoin_bridge_counter!("token_price_updated");
            }

            StarcoinBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_starcoin_bridge_counter!("limit_updated");
            }
        }
    }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthStarcoinBridgeEvents(event) => match event {
                EthStarcoinBridgeEvents::TokensClaimedFilter(_) => (),
                EthStarcoinBridgeEvents::TokensDepositedFilter(_) => (),
                EthStarcoinBridgeEvents::PausedFilter(_) => bump_eth_counter!("bridge_paused"),
                EthStarcoinBridgeEvents::UnpausedFilter(_) => bump_eth_counter!("bridge_unpaused"),
                EthStarcoinBridgeEvents::UpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthStarcoinBridgeEvents::InitializedFilter(_) => {
                    bump_eth_counter!("bridge_contract_initialized")
                }
                EthStarcoinBridgeEvents::ContractUpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthStarcoinBridgeEvents::EmergencyOperationFilter(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: StarcoinClientInner>(
    starcoin_bridge_client: Arc<StarcoinClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            starcoin_bridge_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: StarcoinClientInner>(
    starcoin_bridge_client: Arc<StarcoinClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: StarcoinClientInner>(
    starcoin_bridge_client: Arc<StarcoinClient<C>>,
...
mod tests {
    ...
    use starcoin_bridge_types::bridge::BridgeCommitteeSummary;
    use starcoin_bridge_types::bridge::MoveTypeCommitteeMember;
    use starcoin_bridge_types::crypto::get_key_pair;

    use crate::{starcoin_bridge_mock_client::StarcoinMockClient, types::BridgeCommittee};
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        starcoin_bridge_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        starcoin_bridge_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                starcoin_bridge_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            starcoin_bridge_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                starcoin_bridge_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        starcoin_bridge_monitor_tx
            .send(StarcoinBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        starcoin_bridge_client_mock.set_is_bridge_paused(event.frozen);
        starcoin_bridge_monitor_tx
            .send(StarcoinBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_starcoin_bridge_token_type_tags() {
        let (
            starcoin_bridge_monitor_tx,
            starcoin_bridge_monitor_rx,
            _eth_monitor_tx,
            eth_monitor_rx,
            _starcoin_bridge_client_mock,
            starcoin_bridge_client,
            bridge_pause_tx,
            _bridge_pause_rx,
            authorities,
        ...
        let starcoin_bridge_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let starcoin_bridge_token_type_tags_clone = starcoin_bridge_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                starcoin_bridge_client.clone(),
                starcoin_bridge_monitor_rx,
                ...
                agg.clone(),
                bridge_pause_tx,
                starcoin_bridge_token_type_tags_clone,
                bridge_metrics,
            )
            .run(),
        );
        ...
        starcoin_bridge_monitor_tx
            .send(StarcoinBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in starcoin_bridge_token_type_tags
        starcoin_bridge_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
