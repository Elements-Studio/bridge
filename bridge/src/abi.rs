...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToStarcoinBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, StarcoinToEthBridgeAction,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthStarcoinBridge, EthStarcoinBridgeEvents, "abi/starcoin_bridge.json",
...
pub use eth_starcoin_bridge::EthStarcoinBridge;
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthStarcoinBridgeEvents(event) => {
                match event {
                    EthStarcoinBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToStarcoinTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.starcoin_bridge_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToStarcoinTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };

                        Some(BridgeAction::EthToStarcoinBridgeAction(
                            EthToStarcoinBridgeAction {
                                eth_tx_hash,
                                eth_event_index,
                    ...
                    EthStarcoinBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthStarcoinBridgeEvents::PausedFilter(_event) => None,
                    EthStarcoinBridgeEvents::UnpausedFilter(_event) => None,
                    EthStarcoinBridgeEvents::UpgradedFilter(_event) => None,
                    EthStarcoinBridgeEvents::InitializedFilter(_event) => None,
                    EthStarcoinBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthStarcoinBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
pub struct EthToStarcoinTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub starcoin_bridge_adjusted_amount: u64,
}
...
impl TryFrom<StarcoinToEthBridgeAction> for eth_starcoin_bridge::Message {
    type Error = BridgeError;

    fn try_from(action: StarcoinToEthBridgeAction) -> BridgeResult<Self> {
        Ok(eth_starcoin_bridge::Message {
            ...
            chain_id: action.starcoin_bridge_event.starcoin_bridge_chain_id as u8,
            payload: action
                .as_payload_bytes()
...
impl From<ParsedTokenTransferMessage> for eth_starcoin_bridge::Message {
    fn from(parsed_message: ParsedTokenTransferMessage) -> Self {
        eth_starcoin_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
...
impl TryFrom<EmergencyAction> for eth_starcoin_bridge::Message {
    type Error = BridgeError;
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_starcoin_bridge::Message = action.try_into().unwrap();
        assert_eq!(
            message,
            eth_starcoin_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::StarcoinTestnet,
