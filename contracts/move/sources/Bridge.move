...
module Bridge::Bridge {
    ...
    public fun create(bridge_admin: &signer, chain_id: u8) {
    ...
    public entry fun initialize_bridge(bridge_admin: signer, node_chain_id: u8) {
        assert!(Signer::address_of(&bridge_admin) == @Bridge, ENotSystemAddress);
    ...
    public entry fun register_committee_member(
        ...
        committee_registration(bridge, Signer::address_of(&bridge_admin), bridge_pubkey_bytes, http_rest_url);
    }

    /// Create committee with a single validator (for dev/test)
    public entry fun create_committee(
        bridge_admin: signer,
        ...
        let bridge = borrow_global_mut<Bridge>(@Bridge);
        let validators = SimpleMap::create<address, u64>();
        SimpleMap::add(&mut validators, validator_address, voting_power);
        init_bridge_committee(bridge, validators, min_stake_percentage, epoch);
    }
    ...
    public fun committee_registration(
        ...
        Committee::register(&mut inner.committee, sender, bridge_pubkey_bytes, http_rest_url);
    }

    public fun update_node_url(bridge: &mut Bridge, sender: address, new_url: vector<u8>) {
        Committee::update_node_url(&mut Self::load_inner_mut(bridge).committee, sender, new_url);
    }
    ...
    public fun register_foreign_token<T: store>(
        ...
        Treasury::register_foreign_token(&mut load_inner_mut(bridge).treasury, bridge_admin, mint_cap, burn_cap);
    }
