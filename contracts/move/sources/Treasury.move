...
module Bridge::Treasury {
    ...
    public fun wating_room(t: &BridgeTreasury): &SimpleMap<vector<u8>, ForeignTokenRegistration> {
        &t.waiting_room
    }
    ...
    public fun burn_token<T: store>(token: Token::Token<T>) acquires BridgeTreasuryCap {
        if (Token::value(&token) <= 0) {
            ...
            let tcaps = borrow_global<BridgeTreasuryCap<T>>(@Bridge);
            Token::burn_with_capability(&tcaps.burn_cap, token);
        }
    }
