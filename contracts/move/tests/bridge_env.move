...
module Bridge::BridgeEnv {
    ...
    public fun register_committee(env: &mut BridgeEnv, bridge: &mut Bridge, sender: address) {
        let len = Vector::length(&env.validators);
        let i = 0;
        while (i < len) {
            let vi = Vector::borrow(&mut env.validators, i);
            Bridge::committee_registration(bridge, sender, vi.public_key, b"");
        };
    }

    // Init the bridge committee
    public fun init_committee(_env: &mut BridgeEnv, bridge: &mut Bridge) {
        let voting_powers = CommitteeTest::validator_voting_powers_for_testing();
    ...
    fun register_default_tokens(env: &mut BridgeEnv, bridge: &mut Bridge, bridge_admin: &signer) {
        ...
        // BTC
        let (mint_cap, burn_cap) = AssetUtil::initialize<BTC>(bridge_admin, precision);
        Self::register_foreign_token<BTC>(bridge_admin, bridge, env, mint_cap, burn_cap);

        // ETH
        let (mint_cap, burn_cap) = AssetUtil::initialize<ETH>(bridge_admin, precision);
        Self::register_foreign_token<ETH>(bridge_admin, bridge, env, mint_cap, burn_cap);

        // USDT
        let (mint_cap, burn_cap) = AssetUtil::initialize<USDT>(bridge_admin, precision);
        Self::register_foreign_token<USDT>(bridge_admin, bridge, env, mint_cap, burn_cap);

        // USDC
        let (mint_cap, burn_cap) = AssetUtil::initialize<USDC>(bridge_admin, precision);
    ...
    fun add_default_tokens(env: &mut BridgeEnv, bridge: &mut Bridge) {
        ...
        let signatures = Self::sign_message(env, add_token_message);
        Bridge::execute_system_message(bridge, add_token_message, signatures);
    }
    ...
    public fun update_bridge_limit(
        ...
        let signatures = Self::sign_message(env, msg);

        // run limit update
        Bridge::execute_system_message(bridge, msg, signatures);
    ...
    public fun update_asset_price(
        ...
        // run price update
        Bridge::execute_system_message(bridge, message, signatures);
        //
        // // verify price events
    ...
    public fun execute_blocklist(
        ...
        let signatures = Self::sign_message(env, blocklist);

        // run blocklist
        Bridge::execute_system_message(bridge, blocklist, signatures);
    }
