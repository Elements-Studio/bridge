...
/// This should only be called once from `starcoin_system`.
fun starcoin_system_state(ctx: &TxContext): UID {
...
public(package) fun starcoin_deny_list_object_id(): UID {
    UID {
        id: ID { bytes: SUI_DENY_LIST_OBJECT_ID },
    }
}

public(package) fun starcoin_accumulator_root_object_id(): UID {
    UID {
        id: ID { bytes: SUI_ACCUMULATOR_ROOT_OBJECT_ID },
    }
}

public(package) fun starcoin_accumulator_root_address(): address {
    SUI_ACCUMULATOR_ROOT_OBJECT_ID
}

/// Create the `UID` for the singleton `CoinRegistry` object.
/// This should only be called once from `coin_registry`.
public(package) fun starcoin_coin_registry_object_id(): UID {
    UID {
        id: ID { bytes: SUI_COIN_REGISTRY_OBJECT_ID },
    }
}

public(package) fun starcoin_coin_registry_address(): address {
