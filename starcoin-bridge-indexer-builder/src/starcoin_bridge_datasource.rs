...
use std::sync::Arc;
use starcoin_bridge_data_ingestion_core::{
    DataIngestionMetrics, IndexerExecutor, ProgressStore, ReaderOptions, Worker, WorkerPool,
};
use starcoin_bridge_sdk::StarcoinClient;
use starcoin_bridge_types::full_checkpoint_content::CheckpointData as StarcoinCheckpointData;
use starcoin_bridge_types::full_checkpoint_content::CheckpointTransaction;
use starcoin_bridge_types::messages_checkpoint::CheckpointSequenceNumber;
...
pub struct StarcoinCheckpointDatasource {
    remote_store_url: String,
    starcoin_bridge_client: Arc<StarcoinClient>,
    concurrency: usize,
...
impl StarcoinCheckpointDatasource {
    pub fn new(
        remote_store_url: String,
        starcoin_bridge_client: Arc<StarcoinClient>,
        concurrency: usize,
...
#[async_trait]
impl Datasource<CheckpointTxnData> for StarcoinCheckpointDatasource {
    async fn start_data_retrieval(
        ...
        tracing::info!(
            "Starting Starcoin checkpoint data retrieval with batch size {}",
...
impl Worker for IndexerWorker<CheckpointTxnData> {
    ...
    async fn process_checkpoint(&self, checkpoint: &StarcoinCheckpointData) -> anyhow::Result<()> {
        tracing::trace!(
            "Received checkpoint [{}] {}: {}",
