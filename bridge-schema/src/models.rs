...
use starcoin_bridge_field_count::FieldCount;
use starcoin_bridge_indexer_builder::{Task, LIVE_TASK_TARGET_CHECKPOINT};

use crate::schema::{
    governance_actions, progress_store, starcoin_bridge_error_transactions, starcoin_bridge_progress_store, token_transfer,
    token_transfer_data,
};
...
#[derive(Queryable, Selectable, Insertable, Identifiable, Debug)]
#[diesel(table_name = starcoin_bridge_progress_store, primary_key(txn_digest))]
pub struct StarcoinProgressStore {
...
#[diesel(table_name = starcoin_bridge_error_transactions, primary_key(txn_digest))]
pub struct StarcoinErrorTransactions {
    pub txn_digest: Vec<u8>,
    pub sender_address: Vec<u8>,
    pub timestamp_ms: i64,
